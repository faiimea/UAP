...
def framework_building(fv, v, fp_gene_points, indep_models_path, indep_models_archi, homo_models_path, 
                       homo_models_archi, train_val_split_rate = 0.8, optimizer = None,
    ...
    # Here to define data format of input
    ...
    for cnt in range(len(indep_models_path)):
        ...
        # compare_datasets(data_set, data_set_cur)

        # print(data_set_cur[10][0])
        # print('---')
    ...
    #     inputs, labels = inputs.to(device), labels.to(device)
        ...
        # print(inputs[idx_indep,:][0].shape)
        # print(inputs[idx_indep,:][0])
    ...
    # HERE! Need to change input_ch?
    c = classifier(input_channel=3000).to(device)
    if optimizer is None:
        optimizer = optim.Adam(c.parameters(), lr=5e-4)
    # n_epoch to be 10?
    n_epochs = 10
